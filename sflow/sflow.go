...
func ParseSampleHeader(buf []byte) *SampleDataHeader {
	log.Debugf("%v\n", buf)
...
func ParseIfCounters(buf []byte) *IfCounters {
...
func ParseEthernetCounters(buf []byte) *EthernetCounters {
...
func ParseVlanCounters(buf []byte) *VlanCounters {
...
func main() {
	...
	for {
		...
		go DatagramHandler(buf)
	}
}

func DatagramHandler(buf []byte) {
	sFlowHeaders, err := ParseHeaders(buf)
	if CheckError(err) {
		return
	}
	...
	if sFlowHeaders.version != 5 {
		...
		ParseVersionFive(buf)
	}
}

func ParseFlowRecord(buf []byte) error {
	for len(buf) > 0 {
		...
		if dataFormat == 1 {
			...
			log.Debugf("Flow Buffer: %v", flowBuf)
		}
		if dataFormat == 1001 {
...
func ParseVersionFive(buf []byte) {
	for len(buf) > 0 {
...
func ParseTypeTwo(buf []byte) {
	...
	for len(buf) > 0 {
		...
		if sFlowType == 1 {
			...
			ifCounters := ParseIfCounters(buf)
			...
			ethernetCounters := ParseEthernetCounters(buf)
			...
			vlanCounters := ParseVlanCounters(buf)
